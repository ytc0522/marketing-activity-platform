...
public abstract class RedisLotteryAwardPool implements ILotteryAwardPool {
    ...
    public void refreshPool(LotteryActivityRich lotteryActivityRich) {
        ...
        Long lotteryId = lotteryActivityRich.getLotteryActivity().getId();
        ...
        try {
            ...
            detailList.forEach((e) -> {
                // 初始化 奖池
                redisUtil.zSetAdd(lotteryPoolKey, e.getAwardId(), e.getProbabilityRate().doubleValue());
            });

            // 如果奖品的总概率不足1，则需要补充空的奖品填充到总概率为1。不然概率就会不准确
            BigDecimal totalRate = detailList.stream().map(LotteryAwardDto::getProbabilityRate).reduce(BigDecimal.ZERO, BigDecimal::add);
